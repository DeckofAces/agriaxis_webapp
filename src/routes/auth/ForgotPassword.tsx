  ...
  return (
    <div className="max-w-5/12 min-w-135 rounded-3xl bg-white p-16 flex flex-col gap-10">
      <header className="space-y-2">
        <h5 className="font-neue text-2xl font-semibold text-[#130B30]">Forgot Password</h5>
        <h6 className="text-[#423C59]">Enter your registered {loginType} to reset your password</h6>
      </header>
      <section>
      ...
      <section className="mb-2 space-y-4">
        ...
        <Button variant="link" onClick={() => setLoginType(loginType === 'phone number' ? 'email' : 'phone number')}>
          use my {loginType === 'phone number' ? 'email' : 'phone number'}
        </Button>
      </section>
    </div>
  )
